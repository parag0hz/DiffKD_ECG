#!/bin/bash
# Class-Adaptive KD for 100Hz student.
#
# Requires existing baseline metrics on VALIDATION set.
# Re-evaluates baseline on val set if needed, then trains with class weights.
#
# Usage (from ~/sci):
#   DATA_DIR=comper_repo/ptb_xl bash dafd_mvkt/scripts/run_class_adaptive_100hz.sh
#
# Environment variables:
#   DATA_DIR       path to PTB-XL root
#   TEACHER_CKPT   teacher checkpoint
#   TA_CKPT        TA checkpoint
#   LEAD           lead (default: II)
#   SEED           seed (default: 0)
#   BATCH_SIZE     batch size (default: 256)
#   WEIGHT_MODE    class weight mode (default: inverse_baseline_auc)
...
echo "========================================================"
echo "Class-Adaptive KD 100Hz  lead=${LEAD}  seed=${SEED}"
...
# ── Step 1: Get baseline val metrics ─────────────────────────────────────────
echo ""
...
if [ -f "${BCE_VAL_METRICS}" ]; then
    ...
    if [ ! -f "${BCE_CKPT}" ]; then
        ...
        echo "  Run run_ta_hier_ablation_100hz.sh first."
        exit 1
    fi
...
# ── Step 2: Get TA val metrics ────────────────────────────────────────────────
TA_NAME="ta_ii_500hz"
TA_VAL_METRICS="${OUT_DIR}/metrics_val_${TA_NAME}.json"
TA_EVAL_CKPT="${OUT_DIR}/ta_ii_500hz_best.pt"
# Also check the default TA location
if [ ! -f "${TA_EVAL_CKPT}" ]; then
    TA_EVAL_CKPT="${TA_CKPT}"
fi

if [ ! -f "${TA_VAL_METRICS}" ] && [ "${WEIGHT_MODE}" = "ta_student_gap" ]; then
    echo "  Step 2: TA val metrics"
    python dafd_mvkt/evaluate.py \
...
echo ""
echo "  Step 3: Computing class weights (mode=${WEIGHT_MODE})"
if [ -f "${WEIGHTS_JSON}" ]; then
    echo "  Already exists: ${WEIGHTS_JSON}"
else
    EXTRA_ARGS=""
    if [ "${WEIGHT_MODE}" = "ta_student_gap" ]; then
        EXTRA_ARGS="--ta_metrics ${TA_VAL_METRICS}"
    fi
...
# ── Step 4: Train with class-adaptive weights ─────────────────────────────────
run_adaptive() {
    local LOSSES=$1
    local LOSSES_FNAME
...
echo ""
echo "══ Step 4: Training with class-adaptive weights ══"
run_adaptive "bce,ta_mkd,ta_crf,feature"
run_adaptive "bce,gated_kd,ta_crf,feature"

echo ""
echo "All class-adaptive 100Hz runs complete."
