#!/usr/bin/env bash
# Run all 6 skip-fork-join dual distillation experiments.
#
# Usage:
#   DATA_DIR=comper_repo/ptb_xl SEED=0 LEAD=II REUSE_BRANCHES=0 \
#   bash dafd_mvkt/scripts/run_skip_fork_6.sh
...
echo "=============================================="
echo "  Skip-Fork-Join Dual Distillation Screening"
...
# ── Step 3: run 6 skip-fork experiments ───────────────────────────────────
SFS=(SF01 SF02 SF03 SF04 SF05 SF06)

echo ""
echo "Step 3: Running 6 skip-fork experiments …"
...
for SF in "${SFS[@]}"; do
    ...
    FINAL_METRICS="${OUT_DIR}/metrics_test_student_II_50hz_skipfork_${SF}_${T1}_to_${T2}_${T3}_simclr_seed${SEED}.json"
...
echo ""
echo "=============================================="
echo "  All 6 skip-fork experiments complete."
echo "=============================================="
echo ""
echo "Results:"
cat "${OUT_DIR}/skip_fork_6_results.md" 2>/dev/null || true
echo ""
cat "${OUT_DIR}/skip_fork_analysis.md"  2>/dev/null || true
