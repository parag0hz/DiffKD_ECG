"""
Progressive 1L100→1L50 KD with EMA teacher regularization.

Loss:
    L = BCE
      + lambda_kd  * KD(1L100 → S)
...
Usage:
    python dafd_mvkt/train_student_progressive_ema.py \\
        ...
        --teacher_ckpt    dafd_mvkt/outputs/ta_II_100hz_from_ta500_seed0_best.pt \\
...
TEACHER_IN_CH  = 1    # 1L100: single lead II at 100Hz
STUDENT_IN_CH  = 1    # 1L50:  single lead II at 50Hz
TEACHER_KEY    = "teacher_1l_100"
STUDENT_KEY    = "student_x"       # II lead 50Hz — same key used by 3T-EMA trainer
...
def train(args: argparse.Namespace) -> None:
    ...
    decay_str = str(ema_decay).replace(".", "")
    lam_str   = str(lambda_ema).replace(".", "")
    out_name  = args.output_name or (
        f"student_II_50hz_prog_1l100_ema_lam{lam_str}_decay{decay_str}_seed{seed}")
    ...
    print(f"\n{'='*60}")
    print(f"Progressive EMA:  1L100 → 1L50 student")
    ...
    # ── frozen 1L100 teacher ──────────────────────────────────────────────────
    teacher = _build_model(TEACHER_IN_CH).to(device)
    load_checkpoint(args.teacher_ckpt, teacher, device)
    teacher.eval()
    for p in teacher.parameters():
        p.requires_grad_(False)
    print(f"Teacher (1L100): {args.teacher_ckpt}")

    # ── 1L50 student with optional SimCLR init ────────────────────────────────
    student = _build_model(STUDENT_IN_CH).to(device)
    if args.student_init_ckpt and Path(args.student_init_ckpt).exists():
        ...
        if args.student_init_ckpt:
            print(f"[WARN] SimCLR init not found, random init: {args.student_init_ckpt}")
        else:
    ...
    if args.debug:
        ...
        print(f"\n[DEBUG] student key={STUDENT_KEY}: {first[STUDENT_KEY].shape}")
        print(f"[DEBUG] teacher key={TEACHER_KEY}:  {first[TEACHER_KEY].shape}")
        with torch.no_grad():
            t_out = teacher(first[TEACHER_KEY].to(device), return_features=False)
            s_out = student(first[STUDENT_KEY].to(device), return_features=False)
    ...
    for epoch in range(1, ep + 1):
        student.train()
        ep_bce = ep_kd = ep_kd_ema = ep_tot = 0.0
        n_batches = 0
        ...
        for batch in tqdm(train_loader, desc=f"Ep {epoch}/{ep}", dynamic_ncols=True, leave=False):
            ...
            with torch.no_grad():
                t_out    = teacher(batch[TEACHER_KEY].to(device), return_features=False)
            ...
            loss_kd  = multi_label_kd_loss(s_logits, t_logits,  temperature)
        ...
        if is_best:
            ...
            save_checkpoint(ckpt_path, student, epoch, {"val_auc": val_auc},
                            {"lambda_kd": lambda_kd, "lambda_ema": lambda_ema,
                             "ema_decay": ema_decay})
    ...
    metrics = {
        ...
        "_meta": {"seed": seed, "lambda_kd": lambda_kd, "lambda_ema": lambda_ema,
                  ...
                  "student_init":  str(args.student_init_ckpt)},
    }
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--teacher_ckpt",       required=True,
                   help="1L100 checkpoint, e.g. dafd_mvkt/outputs/ta_II_100hz_from_ta500_seed0_best.pt")
    p.add_argument("--student_init_ckpt",  default=None,
                   help="SimCLR encoder init, e.g. dafd_mvkt/outputs/simclr/simclr_II_50hz_seed0_encoder.pt")
